  ...
  const handleSearch = async (query) => {
    ...
    try {
      const response = await api.get(
        `/api/membership-plans/search?q=${encodeURIComponent(query)}`,
      );
      setPlans(response.data.membership_plans || []);
    } catch (err) {
      console.error(err);
                ...
                {filteredPlans.map((plan) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <span
                        className={`inline-flex px-2 py-1 rounded-full text-xs font-semibold border ${getStatusColor(plan.status)}`}
                      >
                        {plan.status}
                      </span>
                    </td>
