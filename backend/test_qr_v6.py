"""
Test script to fix gym status to 'Active' (capital A)
"""
import pymysql
...
def fix_gym_status():
    """Fix gym status to 'Active' (capital A) and reset password"""
    try:
        # Parse database URL
        ...
        if result:
            ...
            print(f"Gym Status (before): {gym_status}")
            
            # Fix gym status to 'Active' (capital A)
            cursor.execute("""
                UPDATE gyms 
                SET status = 'Active' 
                WHERE id = %s
            ...
            connection.commit()
            print(f"✓ Gym status set to 'Active'")
...
if __name__ == "__main__":
    print("=" * 60)
    print("Fix Gym Status to 'Active'")
    print("=" * 60)
    
    email, gym_id = fix_gym_status()
